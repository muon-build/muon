...
#define WKSTR(cstring)                                                         \
  (struct str) { .s = cstring, .len = strlen(cstring) }
...
void sbuf_init(struct sbuf *sb, char *initial_buffer,
               uint32_t initial_buffer_cap, enum sbuf_flags flags);
...
void sbuf_pushn(struct workspace *wk, struct sbuf *sb, const char *s,
                uint32_t n);
...
obj str_split(struct workspace *wk, const struct str *ss,
              const struct str *split);
obj str_strip(struct workspace *wk, const struct str *ss,
              const struct str *strip);
obj str_split_strip(struct workspace *wk, const struct str *ss,
                    const struct str *split, const struct str *strip);
