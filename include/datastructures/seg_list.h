...
#define sl_get(__sl, __i, __type) (__type *)sl_get_((struct slist *)__sl, __i, sizeof(__type))
...
#define sl_grow_to(__a, __sl, __size, __type) sl_grow_to_(__a, (struct slist *)__sl, __size, sizeof(__type), ar_alignof(__type), ARRAY_LEN((__sl)->segments))

void *sl_alloc_(struct arena *a, struct slist *sl, uint32_t item_size, uint32_t item_align, uint32_t max_segments);
#define sl_alloc(__a, __sl, __type) (__type *)sl_alloc_(__a, (struct slist *)__sl, sizeof(__type), ar_alignof(__type), ARRAY_LEN((__sl)->segments))

void *sl_push_(struct arena *a, struct slist *sl, const void *e, uint32_t item_size, uint32_t item_align, uint32_t max_segments);
#define sl_push(__a, __sl, __e, __type) (__type *)sl_push_(__a, (struct slist *)__sl, __e, sizeof(__type), ar_alignof(__type), ARRAY_LEN((__sl)->segments))

void sl_clear_(struct slist *sl);
#define sl_clear(__sl) sl_clear_((struct slist *)__sl)
void sl_memset_(struct slist *sl, uint8_t c);
#define sl_memset(__sl, __c) sl_memset_((struct slist *)__sl, __c)
