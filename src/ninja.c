...
write_compiler(FILE *file)
{
	...
	if (WEXITSTATUS(rc) == 127) {
		LOG_W(log_misc, "'%s' is not a valid compiler", cc);
	}
...
write_targets(FILE *file, struct context *ctx)
{
	...
	for (size_t i = 0; i < ctx->build.n; ++i) {
		...
		for (size_t j = 0; j < target->include.n; ++j) {
			...
			if (r < 0) {
				LOG_W(log_misc, "failed to compose inclues");
			}
		}
...
emit_ninja(struct context *ctx, const char *build_dir)
{
	if (mkdir(build_dir, 0775) == -1) {
		LOG_W(log_misc, "Build directory already configured");
		return 1;
	}
	LOG_W(log_misc, "Build dir: %s", build_dir);
	...
	if (!ninja) {
		LOG_W(log_misc, "failed to create build.ninja");
		return 1;
	}
