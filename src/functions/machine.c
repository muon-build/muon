...
FUNC_IMPL(machine, system, FUNC_IMPL_RETURN(enum machine_system), .flags = func_impl_flag_impure )
{
	...
	*res = vm_enum_to_obj(wk, enum machine_system, m->sys);
	return true;
}

FUNC_IMPL(machine, subsystem, FUNC_IMPL_RETURN(enum machine_subsystem), .flags = func_impl_flag_impure )
{
	if (!pop_args(wk, NULL, NULL)) {
		return false;
	}
	...
	*res = vm_enum_to_obj(wk, enum machine_subsystem, m->subsystem);
	return true;
}

FUNC_IMPL(machine, endian, FUNC_IMPL_RETURN(enum endianness), .flags = func_impl_flag_impure)
{
	if (!pop_args(wk, NULL, NULL)) {
		return false;
	}
	...
	*res = vm_enum_to_obj(wk, enum endianness, m->endianness);
	return true;
}
...
{
	{
		if (vm_enum(wk, enum endianness)) {
			vm_enum_value_(wk, "enum endianness", "big", big_endian);
			vm_enum_value_(wk, "enum endianness", "little", little_endian);
		};

		if (vm_enum(wk, enum machine_system)) {
#define MACHINE_ENUM(id) vm_enum_value_prefixed(wk, machine_system, id);
			FOREACH_MACHINE_SYSTEM(MACHINE_ENUM)
#undef MACHINE_ENUM
		}

		if (vm_enum(wk, enum machine_subsystem)) {
