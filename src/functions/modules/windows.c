...
func_module_find_resource_compiler(struct workspace *wk, struct module_windows_ctx *ctx)
{
	...
	if (obj_dict_geti(wk, current_project(wk)->compilers, compiler_language_c, &objcomp)) {
		...
		cc_type = get_obj_compiler(wk, objcomp)->type;
	...
	if (!rc_str) {
		switch (cc_type)
		{
			...
			case compiler_clang:
				...
				break;
			default:
				LOG_E("Could not find appropriate environment for resource compiler");
				return false;
		}
	}
	...
	if (run_cmd_argv(&run_cmd_ctx, run_cmd, NULL, 0)) {
		...
		LOG_E("Could not find available resource compiler");
		return false;
	}
