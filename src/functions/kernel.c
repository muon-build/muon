...
add_test_dep_lib_path_to_env(struct workspace *wk, struct obj_test *t, struct obj_build_target *tgt)
{
	...
	if (machine_definitions[tgt->machine]->sys == machine_system_haiku) {
		// Unlike LD_LIBRARY_PATH on other systems,
		// Haiku's runtime_loader only falls back to its built-in
		// default library search path when LIBRARY_PATH is completely unset.
		// Once it is set to anything, those defaults are dropped entirely.
		// To avoid this problem, let's add the default value (https://github.com/haiku/haiku/blob/a1eac5f00572c8b996e60d4cfd8248a07d5f1fae/src/system/runtime_loader/runtime_loader.cpp#L107-L109)
		// to LIBRARY_PATH + the value we need.
		val = make_strf(wk, "%s:/boot/system/lib:/boot/system/non-packaged/lib", rel.buf);
	} else {
		val = tstr_into_str(wk, &rel);
	}

	if (!environment_set(wk, t->env, environment_set_mode_prepend, make_str(wk, env_var), val, 0)) {
		return false;
	}
