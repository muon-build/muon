...
static void
ar_block_free(struct ar_block *block)
{
	z_free(block);
}
...
ar_init(struct arena *a, const struct ar_params *params)
{
	*a = (struct arena){ .params = *params };
	...
	if (!a->params.block_size) {
		a->params.block_size = (16 * 1024) - sizeof(struct ar_block);
	}
}
...
retry:
	...
	if (size > ar_block_len_free(a->tail)) {
		...
		struct ar_block *new_block = ar_block_alloc(a->params.block_size, count, objsize, a->tail);
