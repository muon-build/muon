...
static void *get_obj_internal(struct workspace *wk, obj id,
                              enum obj_type type) {
  struct obj_internal *o = bucket_arr_get(&wk->vm.objects.objs, id);
  if (o->t != type) {
    LOG_E("internal type error, expected %s but got %s", obj_type_to_s(type),
          obj_type_to_s(o->t));
    abort();
    return NULL;
  }
  ...
  case obj_func:
  case obj_capture:
    return bucket_arr_get(&wk->vm.objects.obj_aos[o->t - _obj_aos_start],
                          o->val);
  ...
  case obj_disabler:
  case obj_machine:
    LOG_E("tried to get singleton object of type %s", obj_type_to_s(type));
    abort();
...
enum feature_opt_state get_obj_feature_opt(struct workspace *wk, obj fo) {
  ...
  if (state == feature_opt_auto) {
    ...
    if (wk->global_opts &&
      ...
      return *(enum feature_opt_state *)get_obj_internal(wk, opt->val,
                                                         obj_feature_opt);
    } else {
      return state;
    }
...
void set_obj_feature_opt(struct workspace *wk, obj fo,
                         enum feature_opt_state state) {
...
void make_obj(struct workspace *wk, obj *id, enum obj_type type) {
  ...
  *id = wk->vm.objects.objs.len;
  ...
  bucket_arr_push(&wk->vm.objects.objs,
                  &(struct obj_internal){.t = type, .val = val});
#ifdef TRACY_ENABLE
...
void obj_clear(struct workspace *wk, const struct obj_clear_mark *mk) {
  ...
  for (i = mk->obji; i < wk->vm.objects.objs.len; ++i) {
    ...
    if (o->t == obj_string) {
      ss = bucket_arr_get(&wk->vm.objects.obj_aos[obj_string - _obj_aos_start],
                          o->val);
...
static enum iteration_result obj_equal_array_iter(struct workspace *wk,
                                                  void *_ctx, obj val) {
...
static enum iteration_result obj_equal_dict_iter(struct workspace *wk,
                                                 void *_ctx, obj key, obj val) {
  struct obj_equal_iter_ctx *ctx = _ctx;
  obj r;
...
bool obj_array_foreach(struct workspace *wk, obj arr, void *ctx,
...
static enum iteration_result obj_array_foreach_flat_iter(struct workspace *wk,
                                                         void *_ctx, obj val) {
...
bool obj_array_foreach_flat(struct workspace *wk, obj arr, void *usr_ctx,
                            obj_array_iterator cb) {
...
static enum iteration_result obj_array_index_of_iter(struct workspace *wk,
                                                     void *_ctx, obj v) {
...
static enum iteration_result obj_array_index_iter(struct workspace *wk,
                                                  void *_ctx, obj v) {
...
static enum iteration_result obj_array_dup_iter(struct workspace *wk,
                                                void *_ctx, obj v) {
...
static enum iteration_result obj_array_join_iter(struct workspace *wk,
                                                 void *_ctx, obj val) {
...
static enum iteration_result obj_array_flat_len_iter(struct workspace *wk,
                                                     void *_ctx, obj _) {
  uint32_t *len = _ctx;
  ++(*len);
  return ir_cont;
...
static uint32_t obj_array_flat_len(struct workspace *wk, obj arr) {
  uint32_t len = 0;
  obj_array_foreach_flat(wk, arr, &len, obj_array_flat_len_iter);
  return len;
}

bool obj_array_join(struct workspace *wk, bool flat, obj arr, obj join,
                    obj *res) {
  *res = make_str(wk, "");
...
static enum iteration_result obj_array_dedup_iter(struct workspace *wk,
                                                  void *_ctx, obj val) {
...
static int32_t obj_array_sort_wrapper(const void *a, const void *b,
                                      void *_ctx) {
...
void obj_array_sort(struct workspace *wk, void *usr_ctx, obj arr,
                    obj_array_sort_func func, obj *res) {
...
static enum iteration_result obj_array_slice_iter(struct workspace *wk,
                                                  void *_ctx, obj v) {
...
obj obj_array_slice(struct workspace *wk, obj a, int64_t i0, int64_t i1) {
...
bool obj_dict_foreach(struct workspace *wk, obj dict, void *ctx,
  ...
  if (d->flags & obj_dict_flag_big) {
    ...
    struct obj_dict_elem *e =
        bucket_arr_get(&wk->vm.objects.dict_elems, d->data);
...
static enum iteration_result obj_dict_dup_iter(struct workspace *wk, void *_ctx,
                                               obj key, obj val) {
...
static enum iteration_result obj_dict_merge_iter(struct workspace *wk,
                                                 void *_ctx, obj key, obj val) {
  obj *res = _ctx;
 ...
 */
typedef bool((*obj_dict_key_comparison_func)(
    struct workspace *wk, union obj_dict_key_comparison_key *key,
    uint32_t other));

static bool obj_dict_key_comparison_func_string(
    struct workspace *wk, union obj_dict_key_comparison_key *key, obj other) {
  const struct str *ss_a = get_str(wk, other);
  return str_eql(ss_a, &key->string);
}

static bool
obj_dict_key_comparison_func_int(struct workspace *wk,
                                 union obj_dict_key_comparison_key *key,
                                 uint32_t other) {
  return key->num == other;
}

static bool _obj_dict_index(struct workspace *wk, obj dict,
                            union obj_dict_key_comparison_key *key,
                            obj_dict_key_comparison_func comp, obj **res,
                            uint64_t **ures)
...
bool obj_dict_index_strn(struct workspace *wk, obj dict, const char *str,
...
bool obj_dict_index_str(struct workspace *wk, obj dict, const char *str,
                        obj *res) {
...
static void _obj_dict_set(struct workspace *wk, obj dict,
  ...
  if (!d->len) {
    ...
    bucket_arr_push(&wk->vm.objects.dict_elems,
                    &(struct obj_dict_elem){.key = key, .val = val});
    d->data = e_idx;
    d->tail = e_idx;
    ++d->len;
    return;
  ...
  if (!(d->flags & obj_dict_flag_dont_expand) &&
      !(d->flags & obj_dict_flag_big) && d->len >= 15) {
    struct obj_dict_elem *e =
        bucket_arr_get(&wk->vm.objects.dict_elems, d->data);
    uint32_t h_idx = wk->vm.objects.dict_hashes.len;
    struct hash *h =
        bucket_arr_push(&wk->vm.objects.dict_hashes, &(struct hash){0});
    if (d->flags & obj_dict_flag_int_key) {
  ...
  if ((d->flags & obj_dict_flag_big)) {
    ...
    uint32_t e_idx = wk->vm.objects.dict_elems.len;
    bucket_arr_push(&wk->vm.objects.dict_elems, &(struct obj_dict_elem){
                                                    .key = key,
                                                    .val = val,
                                                });

    struct obj_dict_elem *tail =
        bucket_arr_get(&wk->vm.objects.dict_elems, d->tail);
    tail->next = e_idx;
...
static void _obj_dict_del(struct workspace *wk, obj dict,
                          union obj_dict_key_comparison_key *key,
  ...
  bool found = false;
  struct obj_dict_elem *prev,
      *e = bucket_arr_get(&wk->vm.objects.dict_elems, d->data);
...
void obj_dict_del_strn(struct workspace *wk, obj dict, const char *str,
                       uint32_t len) {
  union obj_dict_key_comparison_key key = {.string = {
                                               .s = str,
                                               .len = len,
...
bool obj_iterable_foreach(struct workspace *wk, obj dict_or_array, void *ctx,
                          obj_dict_iterator cb) {
  ...
  case obj_array: {
    return obj_array_foreach(wk, dict_or_array,
                             &(struct obj_iterable_foreach_ctx){
                                 .ctx = ctx,
                                 .cb = cb,
                             },
...
static enum iteration_result obj_clone_array_iter(struct workspace *wk_src,
                                                  void *_ctx, obj val) {
...
static enum iteration_result obj_clone_dict_iter(struct workspace *wk_src,
                                                 void *_ctx, obj key, obj val) {
  struct obj_clone_ctx *ctx = _ctx;
...
bool obj_clone(struct workspace *wk_src, struct workspace *wk_dest, obj val,
               obj *ret) {
  ...
  case obj_file:
    make_obj(wk_dest, ret, t);
    *get_obj_file(wk_dest, *ret) =
        str_clone(wk_src, wk_dest, *get_obj_file(wk_src, val));
    return true;
  case obj_array:
    make_obj(wk_dest, ret, t);
    return obj_array_foreach(
        wk_src, val,
  ...
  case obj_install_target: {
    ...
    if (!obj_clone(wk_src, wk_dest, in->exclude_directories,
  ...
  case obj_configuration_data: {
    make_obj(wk_dest, ret, t);
    struct obj_configuration_data *conf =
                                      get_obj_configuration_data(wk_src, val),
                                  *o =
                                      get_obj_configuration_data(wk_dest, *ret);
...
static void obj_to_s_opts(struct workspace *wk, obj o, struct sbuf *sb,
                          struct obj_to_s_opts *opts);

static void obj_to_s_indent(struct workspace *wk, struct obj_to_s_ctx *ctx) {
  if (!ctx->opts->pretty) {
    return;
  }
...
static void obj_to_s_pretty_newline(struct workspace *wk,
                                    struct obj_to_s_ctx *ctx) {
  if (!ctx->opts->pretty) {
    return;
  }
...
static enum iteration_result obj_to_s_array_iter(struct workspace *wk,
                                                 void *_ctx, obj val) {
...
static enum iteration_result obj_to_s_dict_iter(struct workspace *wk,
                                                void *_ctx, obj key, obj val) {
  struct obj_to_s_ctx *ctx = _ctx;
...
static void obj_to_s_str(struct workspace *wk, struct obj_to_s_ctx *ctx,
                         obj s) {
  sbuf_push(wk, ctx->sb, '\'');
...
static void obj_to_s_opts(struct workspace *wk, obj o, struct sbuf *sb,
  ...
  case obj_python_installation: {
    ...
    if (get_obj_external_program(wk, py->prog)->found) {
      sbuf_pushf(wk, sb, ", language_version: %s",
                 get_cstr(wk, py->language_version));
  ...
  case obj_external_program: {
    ...
    sbuf_pushf(wk, sb, "<%s found: %s", obj_type_to_s(t),
               prog->found ? "true" : "false");
...
bool obj_vasprintf(struct workspace *wk, struct sbuf *sb, const char *fmt,
                   va_list ap) {
...
uint32_t obj_vsnprintf(struct workspace *wk, char *buf, uint32_t len,
...
uint32_t obj_snprintf(struct workspace *wk, char *buf, uint32_t len,
                      const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
...
static void obj_inspect_dep(struct workspace *wk, FILE *out, const char *pre,
                            struct build_dep *dep) {
  obj_fprintf(wk, out, "%slink_language: %s\n", pre,
              compiler_language_to_s(dep->link_language));
